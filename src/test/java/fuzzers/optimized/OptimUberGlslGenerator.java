...
public class OptimUberGlslGenerator {
    // Configuration constants
    private static final int MIN_UNIFORMS = 3;
    private static final int MAX_UNIFORMS = 80;
    private static final int MIN_GLOBALS = 0;
    private static final int MAX_GLOBALS = 40;
    ...
    private static final int MAX_FUNCTION_PARAMS = 40;
    ...
    public OptimUberGlslGenerator(String version) {
    ...
    private void generateCustomFunction() {
        ...
        for (ParameterInfo param : params) {
            ...
            nonUniformVariables.remove(param.name);
        }
    ...
    private GlslStatement generateForLoop(int depth) {
        ...
        nonUniformVariables.add(loopVarName);
        ...
        nonUniformVariables.remove(loopVarName);
        return forLoop;
    }
    ...
    private GlslStatement generateMethodCall() {
                ...
                if ("void".equals(func.returnType)) {
                    ...
                    if (random.nextBoolean() && !nonUniformVariables.isEmpty()) {
                        ...
                        for (String varName : nonUniformVariables) {
                            ...
                            if (varInfo.type.equals(func.returnType)) {
                    ...
                    nonUniformVariables.add(varName);
                    return stmt;
                }
            }
        }
        ...
        if ("void".equals(returnType)) {
            ...
            if (random.nextBoolean() && !nonUniformVariables.isEmpty()) {
                ...
                for (String varName : nonUniformVariables) {
                    VariableInfo varInfo = availableVariables.get(varName);
                    if (varInfo.type.equals(returnType)) {
                        compatibleVars.add(varName);
                    }
                }
    ...
    private GlslValue generateRandomValue(String type, int depth) {
        ...
        if (random.nextInt(100) < 40 && !availableVariables.isEmpty()) {
            ...
            for (VariableInfo varInfo : availableVariables.values()) {
                if (varInfo.type.equals(type)) {
                    matchingVars.add(varInfo.name);
                }
            }

            if (!matchingVars.isEmpty()) {
                String varName = matchingVars.get(random.nextInt(matchingVars.size()));
        ...
        if (random.nextInt(100) < 30 && !functions.isEmpty()) {
            List<FunctionInfo> matchingFuncs = new ArrayList<>();
            for (FunctionInfo f : functions) {
                if (f.returnType.equals(type) && !f.name.equals(currentFunctionName)) {
                    matchingFuncs.add(f);
                }
            }
        ...
        switch (type) {
            ...
            case "sampler2D":
                ...
                List<String> samplerVars = new ArrayList<>();
                for (VariableInfo varInfo : availableVariables.values()) {
                    if (varInfo.type.equals("sampler2D") && varInfo.isUniform) {
                        samplerVars.add(varInfo.name);
                    }
                }
    ...
    private GlslValue generateRandomCondition(int depth) {
        ...
        if (random.nextBoolean() && !functions.isEmpty()) {
            List<FunctionInfo> boolFuncs = new ArrayList<>();
            for (FunctionInfo f : functions) {
                if ("bool".equals(f.returnType) && !f.name.equals(currentFunctionName)) {
                    boolFuncs.add(f);
                }
            }
    ...
    private GlslValue[] generateRandomArguments(FunctionInfo func) {
        ...
        for (int i = 0; i < func.parameters.size(); i++) {
            ...
            if (random.nextBoolean() && !functions.isEmpty() && (argDepth < MAX_ARG_DEPTH)) {
                List<FunctionInfo> matchingFuncs = new ArrayList<>();
                for (FunctionInfo f : functions) {
                    if (f.returnType.equals(param.type) && !f.name.equals(currentFunctionName)) {
                        matchingFuncs.add(f);
                    }
                }
    ...
    private GlslValue[] generateRandomArgumentsForBuiltIn(String funcName) {
        ...
        for (int i = 0; i < argCount; i++) {
            ...
            if (random.nextBoolean() && !functions.isEmpty()) {
                List<FunctionInfo> matchingFuncs = new ArrayList<>();
                for (FunctionInfo f : functions) {
                    if (f.returnType.equals(argTypes[i]) && !f.name.equals(currentFunctionName)) {
                        matchingFuncs.add(f);
                    }
                }
    ...
    private String[] getBuiltInArgTypes(String funcName) {
        switch (funcName) {
            case "texture":
                return new String[]{"sampler2D", "vec2"};
            case "sin":
            case "cos":
    ...
    public static void main(String[] args) {
        OptimUberGlslGenerator generator = new OptimUberGlslGenerator("330 core");
        generator.generate();
