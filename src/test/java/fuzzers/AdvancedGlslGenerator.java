...
public class AdvancedGlslGenerator {
    ...
    private void generateFunctions() {
        ...
        int functionCount = random.nextInt(30) + 2;
    ...
    private void generateCustomFunction() {
        ...
        int paramCount = random.nextInt(40);
        ...
        int statementCount = random.nextInt(100) + 3;
        for (int i = 0; i < statementCount; i++) {
    ...
    private GlslStatement generateForLoop(int depth) {
        ...
        if (random.nextBoolean() && !functions.isEmpty()) {
            // Use a function call in the condition
            FunctionInfo func = functions.get(random.nextInt(functions.size()));
...
//        return generateAssignment();
    ...
    private GlslStatement generateReturnStatement() {
        ...
        String currentFunc = "";
        if (!currentFunctionReturnPoints.isEmpty()) {
            currentFunc = currentFunctionReturnPoints.get(currentFunctionReturnPoints.size() - 1);
        }

        String finalCurrentFunc = currentFunc;
        FunctionInfo func = functions.stream()
            .filter(f -> f.name.equals(finalCurrentFunc))
            .findFirst()
            .orElse(null);
        
        if (func != null && !"void".equals(func.returnType)) {
            // Non-void function - return a value
            currentFunctionReturnPoints.add(currentFunc);
    ...
    private GlslStatement generateExpressionStatement() {
        ...
        // Function call without using the result
    ...
    private GlslValue generateRandomCondition(int depth) {
        ...
        // Sometimes use a function call that returns bool
        if (random.nextBoolean() && !functions.isEmpty()) {
            List<FunctionInfo> boolFuncs = functions.stream()
                .filter(f -> "bool".equals(f.returnType))
                .collect(Collectors.toList());
    ...
    private GlslValue[] generateRandomArguments(FunctionInfo func) {
        ...
        for (int i = 0; i < func.parameters.size(); i++) {
            ...
            // Sometimes use a function call as an argument
            if (random.nextBoolean() && !functions.isEmpty()) {
                List<FunctionInfo> matchingFuncs = functions.stream()
                    .filter(f -> f.returnType.equals(param.type))
                    .collect(Collectors.toList());
    ...
    private GlslValue[] generateRandomArgumentsForBuiltIn(String funcName) {
        ...
        for (int i = 0; i < argCount; i++) {
            ...
            if (random.nextBoolean() && !functions.isEmpty()) {
                int finalI = i;
                List<FunctionInfo> matchingFuncs = functions.stream()
                    .filter(f -> f.returnType.equals(argTypes[finalI]))
                    .toList();
